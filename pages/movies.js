...
export default function MoviesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/discover?type=movie&genreId=${genreMap[selectedGenre]}`)
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div style={{ fontFamily: "Segoe UI, sans-serif", padding: 30, backgroundColor: "#f4f4f4", minHeight: "100vh" }}>
      ...
      <h1 style={{ fontSize: "2rem", marginBottom: 20 }}>Browse Movies by Genre</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 20 }}>
        {results.map(item => (
          <div key={item.id} style={{ backgroundColor: "white", padding: 16, borderRadius: 10, boxShadow: "0 2px 6px rgba(0,0,0,0.1)" }}>
            <img src={item.poster} alt={item.title} style={{ width: "100%", borderRadius: 6, marginBottom: 8 }} />
            <h3>{item.title}</h3>
